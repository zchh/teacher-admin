                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-danger btn-sm" data-toggle="modal"
                                                data-target="#del_{{$data->relation_power_id}}">
                                            移除
                                        </button>

                                        <!-- Modal -->
                                        <div class="modal fade" id="del_{{$data->relation_power_id}}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog" role="document">
                                                ...
                                                <div class="modal-body">
                                                    <form action="/admin_removePowerToAdminPowerGroup"
                                                          method="post">
                                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                        <input type="hidden" name="_token"
                                                               value="{{ csrf_token() }}">
                                                        <input type="hidden" name="relation_group_id"
                                                               value="{{$data->relation_group_id}}">
                                                        <input type="hidden" name="relation_power_id"
                                                               value="{{$data->relation_power_id}}">
                                                        将要移除该权限！
                                                </div>
                                ...
                                @foreach($checkPower as $value1)
                                    @if(!in_array("$value1->power_id",$power_ids))
                                        <h4><input type="checkbox" name="power_id_array[]"
                                                   value="{{$value1->power_id}}"> {{$value1->power_name}}</input></h4>
                                    @endif
                                @endforeach
                            </div>
                            <div class="modal-footer">
