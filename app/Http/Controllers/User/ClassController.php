...
class ClassController extends Controller 
{
     ...
     public function sClass()
    {
       ...
       //  exit();
          ...
          ->join('base_user', 'base_article_class.class_user', '=', 'base_user.user_id') ->get();
   ...
   {
        $inputData = Request::only("class_id","class_name");dump($inputData);
        foreach ($inputData as $Data)
        {
              $count1 = DB::table('base_article_class')->where('class_id', '=', $Data['class_id'])->update('class_name',$Data["class_name"]);   //修改
          if($count1 == 0)
        {
        ...
        $count2 = DB::table('base_article_class')->where('class_id', '=', $Data['class_id'])->update('class_update_date',date('Y-m-d H:i:s'));   //修改
        if($count2 == 0)
        {
             $baseFunc->setRedirectMessage(false, "修改失败", NULL, "/user_sClass");
        }  
